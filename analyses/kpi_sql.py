...
SITE_NORMALIZE = {
    ...
    "Boinville": "Boinville en Mantois",
    "Bonnieres": "Bonnieres sur Seine",
    "Bourbon-L'Archambaut": "Bourbon L Archambaut",
    "Chazey-Bons": "Chazey Bons",
    "Montalieu-Vercieu": "Montalieu Vercieu",
    "Nesle-Normandeuse": "Nesle Normandeuse",
    "Niort Saint Florent": "Niort St Florent",
    "Saint Victor de Morestel": "St Victor de Morestel",
    "Saint-Amour": "St Amour",
    "Sainte Catherine": "Ste Catherine",
    "Sablé sur Sarthe": "Sable sur Sarthe",
    "Trélivan": "Trelivan",
    "Periers" : "Périers",
}
...
def normalize_site_name(site_name: str) -> str:
    ...
    return SITE_NORMALIZE.get(site_name, site_name)
...
def get_last_update_date(table_name: str) -> datetime:
    ...
    try:
        ...
        print(f" Impossible de recuperer la derniere date pour {table_name}: {e}")
...
def build_evi_combo_tables(df: pd.DataFrame) -> dict:
    ...
    evi_long["Site"] = evi_long["Site"].apply(normalize_site_name)
...
def build_durations_daily(df: pd.DataFrame) -> dict:
    ...
    dur_site_daily["Site"] = dur_site_daily["Site"].apply(normalize_site_name)
    ...
    if "PDC" in ok.columns:
        ...
        dur_pdc_daily["Site"] = dur_pdc_daily["Site"].apply(normalize_site_name)
    else:
...
def build_charges_mac(df: pd.DataFrame, mac_lookup: pd.DataFrame) -> pd.DataFrame:
    ...
    if id_col is None:
        raise ValueError("Aucune colonne ID trouvee dans le DataFrame")
    elif id_col != "ID":
    ...
    out["Site"] = out["Site"].apply(normalize_site_name)
    return out
...
def build_multi_attempts_hour(df: pd.DataFrame) -> pd.DataFrame:
    ...
    agg_dict = {
        ...
        "1ere tentative": ("Datetime start", "min"),
        "Derniere tentative": ("Datetime start", "max"),
    }
...
def build_charges_daily_by_site(df: pd.DataFrame) -> pd.DataFrame:
    ...
    base["Site"] = base["Site"].apply(normalize_site_name)
...
def delete_old_data(table_name: str, start_date: datetime):
    ...
    conn = engine_kpi.connect().execution_options(isolation_level="AUTOCOMMIT")
    try:
        if date_col is None:
            conn.execute(text("SET FOREIGN_KEY_CHECKS = 0"))
            try:
                conn.execute(text(f"TRUNCATE TABLE {full_table}"))
                print(f"   Toutes les donnees supprimees de {full_table} (table agregee)")
            except Exception as e:
                print(f"   Erreur TRUNCATE pour {table_name}: {e}")
            finally:
            ...
            try:
                ...
                print(f"   {result.rowcount} lignes supprimees de {full_table} depuis {start_date.strftime('%Y-%m-%d')}")
            except Exception as e:
                print(f"  Erreur DELETE pour {table_name}: {e}")
    finally:
        conn.close()
...
def save_to_indicator(table_dict: dict, incremental: bool = True):
    ...
    for name, df in table_dict.items():
        if name not in TABLES_TO_SAVE:
            print(f"Table ignoree (hors perimetre) : {name}")
            continue
        if not isinstance(df, pd.DataFrame) or df.empty:
            print(f" Table ignoree (vide) : {name}")
            continue
        ...
        if unique_keys:
            ...
            if missing_cols:
                print(
                    f" COLONNES MANQUANTES POUR LE CONTROLE DE DOUBLONS DANS {name.upper()} : {', '.join(missing_cols)}"
                )
            else:
                dup_mask = df.duplicated(subset=unique_keys, keep=False)
                if dup_mask.any():
                    print(
                        f" DOUBLONS DETECTES DANS {name.upper()} SUR ({', '.join(unique_keys)}) : {dup_mask.sum()} LIGNES CONCERNEES"
                    )
        ...
        try:
            ...
            print(f"Table non trouvee ou erreur chargement : {table_name} -> {e}")
            continue
        ...
        with engine_kpi.begin() as conn:
            try:
                ...
                print(
                    f" Table {'mise a jour' if incremental else 'inseree'} : {schema_name}.{table_name} ({len(df_to_save)} lignes)"
                )
            except Exception as e:
...
def main():
    print("=" * 80)
    print("DEMARRAGE DU PROCESSUS DE MISE A JOUR INCREMENTALE")
    print("=" * 80)
    
    last_update = get_last_update_date("sessions")
    print(f"\n Derniere mise a jour detectee : {last_update.strftime('%Y-%m-%d')}")
    print(f" Recuperation des donnees depuis : {last_update.strftime('%Y-%m-%d')}")
    
    df = fetch_charge_data(start_date=last_update)
    print(f" {len(df)} nouvelles charges recuperees")
    
    if df.empty:
        print("Aucune nouvelle donnee a traiter. Arret du processus.")
        return
    
    print(f"\n  Suppression des donnees existantes depuis {last_update.strftime('%Y-%m-%d')}...")
    ...
    if "Name Project" in df.columns:
        df["Name Project"] = df["Name Project"].apply(normalize_site_name)

...
if ok_sum.empty:
    ...
    # conversions
    for c in ("Datetime start", "Datetime end"):
        if c in ok_sum.columns:
            ok_sum[c] = pd.to_datetime(ok_sum[c], errors="coerce")
    if "Energy (Kwh)" in ok_sum.columns:
        ok_sum["Energy (Kwh)"] = pd.to_numeric(ok_sum["Energy (Kwh)"], errors="coerce")
    for c in ("SOC Start", "SOC End"):
        if c in ok_sum.columns:
            ok_sum[c] = pd.to_numeric(ok_sum[c], errors="coerce")
    if "MAC Address" in ok_sum.columns:
        ok_sum["MAC Address"] = ok_sum["MAC Address"].apply(_fmt_mac)

    # petite info utile mais neutre (pas "Erreur")
    def _soc_evo_ok(row):
        s0 = row.get("SOC Start", pd.NA)
        s1 = row.get("SOC End", pd.NA)
        if pd.notna(s0) and pd.notna(s1):
            try:
                ...
                return ""
        return ""
    ok_sum["Évolution SOC"] = ok_sum.apply(_soc_evo_ok, axis=1)

    # Lien ELTO
    if "ID" not in ok_sum.columns:
        st.warning("Colonne 'ID' absente : les liens IEC ne seront pas affichés (charges OK).")
        ok_sum["ELTO"] = ""
    else:
        ok_sum["ELTO"] = BASE_CHARGE_URL + ok_sum["ID"].astype(str).str.strip()

    # colonnes à afficher (sans "Erreur")
    cols_aff_ok = [
        "ID", "Datetime start", "Datetime end", "PDC",
        "Energy (Kwh)", "MAC Address", "Évolution SOC", "ELTO"
    ]
    cols_aff_ok = [c for c in cols_aff_ok if c in ok_sum.columns]

    out_ok = ok_sum[cols_aff_ok].copy()
    if "Datetime start" in out_ok.columns:
        out_ok = out_ok.sort_values("Datetime start", ascending=False)
    out_ok.insert(0, "#", range(1, len(out_ok) + 1))

    st.subheader("Charges OK")
    st.data_editor(
        out_ok,
        use_container_width=True,
        hide_index=True,
        column_config={
            "ELTO": st.column_config.LinkColumn(
                "Lien IEC",
                help="Ouvrir la session dans IEC",
                display_text="🔗 Ouvrir"
            ),
            "Datetime start": st.column_config.DatetimeColumn("Start time", format="YYYY-MM-DD HH:mm:ss"),
            "Datetime end":   st.column_config.DatetimeColumn("End time",   format="YYYY-MM-DD HH:mm:ss"),
